...
class JARVISUI(BoxLayout):
    ...
    def listen_voice_input(self):
        if not HAS_ANDROID:
            self.add_message("Voice input is available in the Android APK.", is_user=False)
            return
    ...
    def voice_permission_callback(self, permissions, grants):
        try:
            if grants and all(grants):
                ...
                self.add_message("Microphone permission was denied.", is_user=False)
        except Exception as e:
            self.add_message(f"Permission error: {e}", is_user=False)
    ...
    def launch_voice_recognizer(self):
        try:
            ...
            self.add_message(f"Voice recognition error: {e}", is_user=False)
        ...
        try:
            if result_code == -1 and intent is not None:
                ...
                if results is not None and results.size() > 0:
                    recognized_text = str(results.get(0))
                    self.ids.command.text = recognized_text
                    Clock.schedule_once(lambda dt: self.ask(), 0.2)
        except Exception as e:
            self.add_message(f"Voice result error: {e}", is_user=False)

    def request_hide_chat_access(self):
        self.awaiting_password = True
        self.add_message("🔒 Enter Security Password:", is_user=False)
    ...
    def open_decoy_chat_panel(self, is_fake=False):
        ...
        if is_fake:
            ...
            fake_messages = [
                {"text": "Hey, bought milk and grocery items?", "is_user": True},
                {"text": "Yes, I got them. Anything else needed?", "is_user": False},
                {"text": "No, that is all. Thanks!", "is_user": True}
            ]
            for msg in fake_messages:
                self.add_message_bubble(msg["text"], msg["is_user"], is_secret=False)
        else:
            self.is_secret_mode = True
            self.is_decoy_panel = False

            self.add_message("⚡ MOSAIC PRIVATE VAULT UNLOCKED ⚡", is_user=False)
            self.add_message("Type 'pin' to keep history, or 'unpin' to auto-clear on exit.", is_user=False)

            for msg in self.secret_chat_history:
                self.add_message_bubble(msg["text"], msg["is_user"], is_secret=True)
    ...
    def exit_secret_mode(self):
        ...
        self.add_message("Exited Private Vault. Back to Main JARVIS.", is_user=False)
    ...
    def save_secret_history(self):
        try:
            with open(SECRET_FILE, "w", encoding="utf-8") as f:
                json.dump(self.secret_chat_history, f, ensure_ascii=False, indent=2)
        except Exception:
            pass
    ...
    def open_three_dots_menu(self, widget):
        ...
        btn_mosaic.bind(
            on_release=lambda b_obj: (
                dropdown.dismiss(),
                self.exit_secret_mode() if self.is_secret_mode else self.request_hide_chat_access()
            )
        )
    ...
    def open_model_dropdown(self, widget):
        ...
        dropdown = DropDown()
        models = ["Gemini Flash", "Gemini Pro", "ChatGPT 4o", "JARVIS Local"]
        ...
        for m_name in models:
            ...
            btn.bind(
                on_release=lambda b_obj, name=m_name: (
                    setattr(self, 'selected_model', name),
                    dropdown.dismiss()
                )
            )
    ...
    def open_sidebar(self):
        popup = Popup(
            title="JARVIS Menu",
            content=Label(text="JARVIS AI System v1.0"),
            size_hint=(0.7, 0.4)
        )
        popup.open()
    ...
    def open_vision_picker(self):
        ...
        btn_select = Button(text="Select Image")
        ...
        popup = Popup(title="Select Document or Image", content=content, size_hint=(0.9, 0.9))

        def load_file(instance):
            if filechooser.selection:
                selected_path = filechooser.selection[0]
                popup.dismiss()
                self.add_message(f"📷 Selected image: {os.path.basename(selected_path)}", is_user=True)
                self.add_message("Analyzing document / image content...", is_user=False)
    ...
    def ask(self):
        ...
        if self.is_secret_mode:
            ...
            if clean_cmd == "pin":
                self.is_pinned = True
                self.save_secret_history()
                self.add_message("📌 Private chat pinned! History will persist.", is_user=False)
                return
            elif clean_cmd == "unpin":
                self.is_pinned = False
                self.add_message("🗑️ Private chat unpinned! History will auto-clear on exit.", is_user=False)
                return

        Clock.schedule_once(lambda dt: self.generate_response(user_text), 0.1)

    def generate_response(self, user_text):
        lower_text = user_text.lower()

        calc_res = SafeCalculator.calculate(user_text)
        ...
        if "hello" in lower_text or "hi" in lower_text:
            response = f"Hello! Operating via {self.selected_model}."
        elif "who are you" in lower_text:
            response = "I am JARVIS, your personal AI assistant."
        else:
            response = f"Processed '{user_text}' using {self.selected_model}."

        self.add_message(response, is_user=False)
        self.speak_result(response)

    def add_message(self, text, is_user=False):
        msg_obj = {"text": text, "is_user": is_user}
        if self.is_secret_mode:
            self.secret_chat_history.append(msg_obj)
            if self.is_pinned:
                self.save_secret_history()
        elif not self.is_decoy_panel:
            self.normal_chat_history.append(msg_obj)

        self.add_message_bubble(text, is_user, is_secret=self.is_secret_mode)

    def add_message_bubble(self, text, is_user, is_secret=False):
        ...
        self.ids.chat_logs.add_widget(bubble)
...
class JARVISApp(App):
    def build(self):
        ...
        try:
            ...
            from kivy.uix.label import Label
            return Label(text=f"Startup Error: {str(e)}")
...
if __name__ == "__main__":
    try:
        ...
        print("Fatal Launch Error:", fatal_err)
